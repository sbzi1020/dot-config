...
#
#     if [ $os_type = "Linux" ]
#         set --local ip (/sbin/ip address show | grep -E "inet .* global" | cut -d " " -f6)
#         set --local gw (/sbin/ip route | grep default | cut -d " " -f3)
...
# ~/my-shell
#--------------------------------------------------------
set PATH ~/my-shell ~/my-shell/lua-language-server/bin /usr/lib/llvm18/bin /sbin /usr/sbin /usr/local/bin $PATH
...
abbr startdocker "doas service docker start"
abbr stopdocker "doas service docker stop"
...
# abbr ns "ss"
abbr nst "netstat -napt"
abbr nstl "netstat -nptl"
abbr nsu "netstat -napu"
...
set --export PYENV_INIT_RESULT (pyenv init --path)
set --export PYENV_INIT_RESULT_2 (pyenv init -)

set PATH "$HOME/.cargo" $PATH

abbr reset-screen-resulotion "wlr-randr --output DP-1 --mode 3840x2160 --scale 1.4 --above eDP-1 &"
