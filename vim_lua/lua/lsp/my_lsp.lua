...
local custom_lsp_attach = function()
    ...
    vim.api.nvim_buf_set_keymap(0, 'n', 'B', '<cmd>lua vim.lsp.buf.hover()<CR>', mapping_options)
...
2. Rust LSP server
    ...
    `brew search rust-analyzer`

For all LSP servers, check here:
https://microsoft.github.io/language-server-protocol/implementors/servers/
--]]
-- local lsp_server_loading_list = { "rust_analyzer", "tsserver" }
local lsp_server_loading_list = { "tsserver" }
for _, lsp in ipairs(lsp_server_loading_list) do
    -- print(lsp)
    nvim_lsp[lsp].setup {
